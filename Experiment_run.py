...
string_values = 'ABC'
#output_dir = 'KASAM_Experiment_A/'
output_dir = 'KASAM_Experiment_' + string_values[experiment_val] +'/'



def f(x,experiment_val):
    # Experiment A
    if experiment_val == 0:
        ...
        xt = 10.*x - 5.
        v0 = np.sin(xt[0])**2+np.sin(xt[1])**2 + 2.*np.exp(-xt[0]**2-xt[1]**2)
...
def f2(x):
    ...
    v0 = f(x,experiment_val)*np.logical_not(l3) + 0.*l3
...
y0 = f(array_grid,experiment_val)
...
class sam(keras.Model):
    ...
    def __init__(self, n, m, c0, u0):
        ...
        self.n0 = self.c0 + self.d0 # the number of control points; capacity + degree of basis function
        self.p0 = self.n0*self.n

        self.w0 = tf.constant_initializer(self.c0*partition_weights(self.n,self.n0))
        self.b0 = tf.constant_initializer(self.d0 - partition_bias(self.n,self.n0))

        self.w1 = tf.constant_initializer(0.)

        #inn = tf.keras.layers.Input(shape=(n,))

        self.l00 = tf.keras.layers.Dense(activation=s,
                                    units=self.p0,
                                    use_bias=True,
                                    trainable=False,
                                    kernel_initializer=self.w0,
                                    bias_initializer=self.b0)

        self.out = tf.keras.layers.Dense(units=self.m,
                                    use_bias=False,
                                    trainable=True,
                                    kernel_initializer=self.w1)
        
    def call(self, input_tensor, training=False):
        
        out = self.out(self.l00(input_tensor))
...
for index in range(0,num_trials):
    ...
    train_output = np.array([f(train_inputs.T,experiment_val)]).T 
...
fig0.savefig(output_dir+"training_time_task_1_show_all.png", dpi=500, bbox_inches = 'tight')
...
fig0.savefig(output_dir+"validation_task_1_show_all.png", dpi=500, bbox_inches = 'tight')
...
fig1.savefig(output_dir+"training_time_task_1.png", dpi=500, bbox_inches = 'tight')
plt.close()
...
fig1.savefig(output_dir+"validation_task_1.png", dpi=500, bbox_inches = 'tight')
plt.close()
...
fig2.savefig(output_dir+"training_time_task_2.png", dpi=500, bbox_inches = 'tight')
...
fig2.savefig(output_dir+"validation_task_2.png", dpi=500, bbox_inches = 'tight')
